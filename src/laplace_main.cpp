...
int main()
{
    ...
    auto result_jacobi = solve_jacobi(u_old, u_new, nx, ny, 1e-4);

    auto end = std::chrono::high_resolution_clock::now();
    auto total_time_ms = std::chrono::duration<double, std::milli>(end - start).count();

    std::cout << "toal_time: " << total_time_ms << "ms\n";
    std::cout << "iteration: " << result_jacobi.iterations << "\n";
    std::cout << "final_diff:" << result_jacobi.final_diff << "\n";
